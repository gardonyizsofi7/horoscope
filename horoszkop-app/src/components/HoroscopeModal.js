...
export const HoroscopeModal = ({ visible, zodiac, timeframe, onClose }) => {
  ...
  const shareHoroscope = async () => {
    try {
      ...
      if (isAvailable) {
        // Since expo-sharing requires a file URI, we might need another approach to share text directly in some setups,
        // but react-native's Share API is better for plain text.
        // I will use react-native's Share API as expo-sharing is mostly for files.
      }
    } catch (error) {
      console.error(error);
    }
